...
def _get_embedding_collection_store(vector_dimension: Optional[int] = None) -> Any:
    ...
    class CollectionStore(BaseModel):
        ...
        @classmethod
        def get_by_id(
            cls, session: Session, id: str
        ) -> Optional["CollectionStore"]:
            return session.query(cls).filter(cls.uuid == id).first()  # type: ignore
        ...
        def get_or_create(
            ...
            """  # noqa: E501
            created = False
            collection = cls.get_by_id(session, id)
            if collection:
                return collection, created

            collection = cls(name=name or _LANGCHAIN_DEFAULT_COLLECTION_NAME, cmetadata=cmetadata, uuid=id)
...
class PGVector(VectorStore):
    ...
    Args:
        ...
        collection_id: The name of the collection to use. (default: langchain)
    ...
    Example:
        .. code-block:: python
            ...
            collection_id = "state_of_the_union_test"
    ...
    def __init__(
        ...
        collection_id: str = _LANGCHAIN_DEFAULT_COLLECTION_NAME,
    ...
    def create_collection(self) -> None:
        ...
        with Session(self._bind) as session:
            self.CollectionStore.get_or_create(
                session, self.collection_id, self.collection_name, cmetadata=self.collection_metadata
            )
    ...
    def get_collection(self, session: Session) -> Any:
        return self.CollectionStore.get_by_id(session, self.collection_id)
